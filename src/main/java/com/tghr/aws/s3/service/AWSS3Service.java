...
public class AWSS3Service {
	...
	public List<String> getCarImageList(final String carId) {
		...
		try {
			...
			fileURIList = fileutil.getCarImagesList(s3Client, fileutil.getFilePrefix(carId));
		} catch (Exception e) {
			logger.info("IO Error Message= " + e.getMessage());
	 ...
	 */
	public void deleteFile(final String uniqueFileName) {
		logger.info("Deleting file with name= " + uniqueFileName);
		final DeleteObjectRequest deleteObjectRequest = new DeleteObjectRequest(bucketName, uniqueFileName);
